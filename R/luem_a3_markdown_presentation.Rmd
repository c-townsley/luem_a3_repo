...
* It displayed high statistical significance for all values except Land Porosity. 

* The AIC was **1259.8**

* The relationships that came forward were, with all else considered equal:
...
** Overall, the model's true positive rate - its _sensitivity_ - is almost twice as much as its false positive rate. 

** The model's true negative rate, is about 15 times greater than its false negative rate, indicating the model's high _specificity_.
...
We also experimented with the threshold cutoff to check how the model would respond, and found minor changes in shifting the cutoff to 75% or 25%. 

Overall, the 50% threshold gave the best balance of accuracy, sensitivity, and specificity. 
...
The Receiver Operating Characteristic (ROC) curve, which shows the trade-off between Sensitivity and Specificity in our test set, confirms that the model is able to predict flood inundation in Calgary with a **94%** accuracy. 
...
A histogram of this shows that all predictions are clustered at high accuracy values, indicating a high level of **generalizability**, that is, a measure of the model's capacity to be applied to predict other sample sets -- which in our case, made us more confident it would work for Denver.
...
An error map spatially laid out our false negatives and positives, leads us to think we may have over-predicted flooding in low elevation areas, and, by initially classifiying land porosity as a binary of land cover, we may have reduced the nuances in soil run-off and flooding. 
