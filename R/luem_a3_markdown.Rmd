...
ggplot() +
  geom_sf(data=calg_dat, aes(fill=streamdist_min), color = NA) +
  scale_fill_gradient(low = yellows[1], high = "#6E5321",
                    name = "Distance\n(Quantile Breaks\nin Meters)") +
...
ggplot() +
  ...
  scale_fill_manual(values = neutrals,
                    labels= as.character(round(quantile(calg_dat$elevation_median,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T), 2)),
                    name = "Mean Elevation\n(Quantile Breaks\nCategorical)") +
