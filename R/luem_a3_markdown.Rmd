...
This project suggests it is important for cities that face the potential of flooding but have limited inundation history to study comparable cities to better understand their own flood exposure.
...
We began by thinking of the factors that could influence and/or correlate with flood inundation. Based on 

We hypothesized that the probability of a grid cell in our model to flood is a function of 

land cover permeability (perviousness?)

Elevation

Where water has a natural tendency to flow based on elevation.
...
ggplot() +
  ...
  scale_fill_manual(values = yellows,
                    labels=as.character(quantile(calg_dat$streamdist_min,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Distance\n(Quantile Breaks\nin Meters)") +
...
ggplot() +
  ...
  scale_fill_manual(values = blues,
                    labels=as.character(quantile(calg_dat$flowac_mean,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Mean Flow\nAccumulation\n(Quantile Breaks)") +
...
ggplot() +
  ...
  scale_fill_manual(values = greens,
                    labels=as.character(quantile(calg_dat$pervious_mean,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Permeable\nSurface\n(Quantile\nBreaks)") +
  labs(title="Permeable Surface in Calgary",
...
ggplot() +
  geom_sf(data=calg_dat, aes(fill=factor(ntile(elevation_median,4))), 
            colour=NA) +
  scale_fill_manual(values = neutrals,
                    labels=as.character(quantile(calg_dat$elevation_median,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Mean Elevation\n(Quantile Breaks\nin Meters)") +
...
ggplot() +
  geom_sf(data=denver_dat, aes(fill=factor(ntile(streamdist_min,4))), 
            colour=NA) +
  scale_fill_manual(values = yellows,
                    labels=as.character(quantile(denver_dat$streamdist_min,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Distance\n(Quantile Breaks\n in Feet)") +
...
ggplot() +
  geom_sf(data=denver_dat, aes(fill=factor(ntile(flowac_mean,4))), 
            colour=NA) +
  scale_fill_manual(values = blues,
                    labels=as.character(quantile(denver_dat$flowac_mean,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Mean Flow\nAccumulation\n(Quantile Breaks)") +
...
ggplot() +
  geom_sf(data=denver_dat, aes(fill=factor(ntile(pervious_mean,4))), 
            colour=NA) +
  scale_fill_manual(values = greens,
                    labels=as.character(quantile(denver_dat$pervious_mean,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Permeable\nSurface\n(Quantile\nBreaks)") +
...
ggplot() +
  geom_sf(data=denver_dat, aes(fill=elevation_median), 
            colour=NA) +
...
* The Aikake Information Criterion (AIC) is a useful number when comparing models on the basis of their performance. When selecting multiple regression models, a lower AIC indicates a better model performance.^3^ In this case, the AIC is **1253.8**.
...
If we assume a probability cutoff threshold of 50%, our confusion matrix gives us an accuracy of **0.917**, with the following sensitivity and specificity results:
...
* It has predicted 0 as 0, i.e. a _True Negative_, 1397 times
* It has predicted 0 as 1, i.e. a _False Negative_, 92 times
* It has predicted 1 as 0, i.e. a _False Positive_, 42 times
* It has predicted 1 as 1, i.e. a _True Positive_, 80 times
...
It has an error rate of about 8% as derived from the accuracy value (Error = Accuracy - 1), which is demonstrated by the fact that the model's 134 erroneous predictions (92+42) are only a small fraction of its 1477 accurate predictions (1397+80).
...
The accuracy of a model with with a 75% threshold cutoff remains almost the same as the original model with a 50% cutoff, at **0.913**
...
Similarly, the accuracy of a model with a 25% cutoff is only marginally higher than the original model at **0.919**, but it also exhibits a lower specificity:
...
This is further confirmed by calculating the area under the curve as **0.947**, which confirms that the model is able to predict flood inundation with a 94.7% accuracy. 
...
calg_dat2 <- calg_dat %>% 
  dplyr::select(uniqueID, inund_sum, pervious_mean, elevation_median, flowac_mean, streamdist_min, geometry)

allPredictions <- 
  predict(inundFit, calg_dat2, type="prob")[,2]
...
```{r predicted_map_denver, warning = FALSE, message = FALSE}
 ...
 ggplot() + 
    geom_sf(data=denver_pred, aes(fill=factor(allPredictions_denver)), 
            colour=NA) +
    scale_fill_manual(values = c("#CEEBF0", "#51A6AE"),
                      labels=c("0", "1"),
                      name="Predicted\nProbabilities\n(Binary)") +
...
1. City of Calgary. “Flooding in Calgary - Flood of 2013.” Https://Www.Calgary.Ca. Accessed March 27, 2023. https://www.calgary.ca/content/www/en/home/water/flooding/history-calgary.html.
...
ggplot() + 
    geom_sf(data=denver_pred, aes(fill=allPredictions), 
            colour=NA) +
    scale_fill_viridis(
                      alpha = 1,
                      begin = 1,
