...
ggplot() +
  ...
  scale_fill_manual(values = neutrals,
                    labels=as.character(quantile(calg_dat$elevation_mean,
                                                 c(0.2,.4,.6,.8),
                                                 na.rm=T)),
                    name = "Mean Elevation\n(Quantile Breaks\nin Meters)") +
...
The accuracy of a model with with a 75% threshold cutoff is a little higher than the original model with a 50% cutoff, at **0.917**
...
This is further confirmed by calculating the area under the curve as **0.939**, which confirms that the model is able to predict flood inundation with a 94% accuracy. 
